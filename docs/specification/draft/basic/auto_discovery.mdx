...
Auto-discovery starts **before** any MCP workflow and finishes **before** the first request is sent.
...
1. URI scheme that an MCP client can use to detect MCP servers and MCP features to use.
...
By requiring a TXT record, we ensure that the domain owner explicitly authorizes any MCP server resolved from this process.
This takes care that no unauthorized servers are being injected.
...
The JWT to discover the MCP server endpoint SHOULD have these claims:
 ...
 - `metadata` (optional):  Map <key, value>
 ...
 - singed by an official CA (X.509 signature chain).
...
1. **TXT Record Lookup**
   ...
   - The client MUST refresh the TXT record after min( TTL_DNS, exp-now ) seconds and
     MUST NOT cache the JWT beyond its exp value.
...
2. **JWT Validation**
   ...
   - The JWT payload includes:
     ...
     - Optional metadata like namespaces or versions.
...
| --- | --- | --- |
| DNS spoofing | DNSSEC validation or DoH + HTTPS fallback | Low |
| Replay of expired TXT | `exp` claim enforced; Clients reject older than _now_ | Very low |
| Server substitution | HTTPS certificate must match one of the hosts in `mcpServers` | Low |
...
- Reduces configuration overhead and removes the need for a dedicated registry.
